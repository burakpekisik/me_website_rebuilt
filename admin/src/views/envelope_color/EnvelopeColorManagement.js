...
import { envelopeColorsApi } from './FetchEnvelopeColors'
import { HexColorPicker } from 'react-colorful'
import EnvelopeColorCreate from './EnvelopeColorCreate'
...
const EnvelopeColorManagement = ({ theme }) => {
  ...
  const handleAddColor = async (colorData) => {
    try {
      await envelopeColorsApi.createEnvelopeColor(colorData)
      fetchColors()
  ...
  const handleCellEdit = async (colorId, field, value) => {
    try {
      ...
      await envelopeColorsApi.updateEnvelopeColor(colorId, updatedData)
      setColors(
  ...
  const fetchColors = async () => {
    try {
      const data = await envelopeColorsApi.getEnvelopeColors()
      setColors(data)
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await envelopeColorsApi.deleteEnvelopeColor(deleteConfirmation.id)
  ...
  return (
    <div>
      <Header>
        <PageTitle>Zarf Renkleri</PageTitle>
                    ...
                    {colorPickerVisible === color.id && (
                      <ColorPickerPopover>
                        <HexColorPicker
                          ...
                          onChange={(newColor) => {
                            handleCellEdit(color.id, 'color_code', newColor)
                          }}
                        />
                      </ColorPickerPopover>
                    )}
...
export default EnvelopeColorManagement
